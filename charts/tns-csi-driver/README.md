...
- **SELinux Mount Labeling**: Optional kubelet-managed per-pod SELinux mount contexts
...
| Parameter | Description | Default |
|-----------|-------------|---------|
| `csiDriverName` | Registered CSI driver name | `tns.csi.io` |
| `csiDriver.seLinuxMount` | Advertise support for kubelet-managed SELinux context mounts (Kubernetes 1.25+) | `false` |

#### SELinux Mount Labeling

The driver passes StorageClass mount options through to the node mount command. A fixed SELinux label can therefore be configured without enabling automatic labeling:

```yaml
storageClasses:
  - name: truenas-iscsi
    # Other StorageClass settings omitted.
    mountOptions:
      - 'context=system_u:object_r:ephemeral_t:s0'
```

Use a label allowed by the SELinux policy installed on your nodes. For labels with MCS categories, preserve the inner quotes so the comma remains part of the `context` option:
...
Kubernetes 1.25 and newer can derive the mount label from a pod's explicitly configured or admission-injected `seLinuxOptions`. A label assigned only by the container runtime is not sufficient. Enable this only after validating the labels generated by your Kubernetes distribution:
...
This renders `CSIDriver.spec.seLinuxMount: true`. Kubernetes 1.25 through 1.27 require the alpha `SELinuxMountReadWriteOncePod` feature gate. Kubernetes 1.28 through 1.36 enable or graduate that behavior for volumes whose sole access mode is `ReadWriteOncePod`; consult the Kubernetes feature-gate documentation before using other access modes. The driver automatically adds `nosharecache` to NFS mounts that receive a `context=` option; SMB mounts already use `nosharesock`.

Keep this setting disabled on platforms whose SELinux policy does not support kubelet-derived labels. For example, Talos uses its own policy and fixed `ephemeral_t` mount options may be more appropriate. Test enforcing-mode behavior before enabling it in production.
