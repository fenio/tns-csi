...
var _ = Describe("NFS Snapshot Stress", func() {
	...
	It("should handle multiple snapshots of the same volume", func() {
		...
		_, err = f.K8s.ExecInPod(ctx, pod.Name, []string{"sh", "-c", "echo 'Initial Data' > /data/initial.txt"})
		...
		for i := range numSnapshots {
			...
			GinkgoWriter.Printf("Snapshot %d/%d created: %s\n", i+1, numSnapshots, snapshotName)
		}
		...
		restore1PVC := "snapshot-stress-restore-1"
		err = f.K8s.CreatePVCFromSnapshot(ctx, restore1PVC, snapshotNames[0], "tns-csi-nfs", "1Gi",
		...
		restoreLastPVC := "snapshot-stress-restore-last"
		err = f.K8s.CreatePVCFromSnapshot(ctx, restoreLastPVC, snapshotNames[numSnapshots-1], "tns-csi-nfs", "1Gi",
			[]corev1.PersistentVolumeAccessMode{corev1.ReadWriteMany})
		...
		restore1Pod, err := f.CreatePod(ctx, framework.PodOptions{
			Name:      "snapshot-stress-restore-pod-1",
			PVCName:   restore1PVC,
			MountPath: "/data",
		})
		...
		restoreLastPod, err := f.CreatePod(ctx, framework.PodOptions{
			Name:      "snapshot-stress-restore-pod-last",
		...
		err = f.K8s.WaitForPodReady(ctx, restoreLastPod.Name, 2*time.Minute)
		Expect(err).NotTo(HaveOccurred())
		...
		GinkgoWriter.Printf("Successfully created and verified %d snapshots\n", numSnapshots)
	})
})

var _ = Describe("NFS Volume Stress", func() {
	var f *framework.Framework
	...
	It("should handle multiple volumes concurrently", func() {
		...
		for _, pvcName := range pvcNames {
			...
			GinkgoWriter.Printf("PVC %s bound\n", pvcName)
		}
		...
		for i := range numVolumes {
			...
			go func(index int) {
				defer wg.Done()
				podName := fmt.Sprintf("stress-pod-%d", index+1)
				...
				_, err := f.K8s.CreatePod(ctx, framework.PodOptions{
					...
					MountPath: "/data",
					Command:   []string{"sh", "-c", fmt.Sprintf("echo 'Pod %d data' > /data/test.txt && sleep 300", index+1)},
				})
				if err != nil {
		...
		for _, podName := range podNames {
			err := f.K8s.WaitForPodReady(ctx, podName, 2*time.Minute)
			Expect(err).NotTo(HaveOccurred(), "Pod %s did not become Ready", podName)
			GinkgoWriter.Printf("Pod %s ready\n", podName)
		}
		...
		GinkgoWriter.Printf("Successfully created and verified %d concurrent volumes\n", numVolumes)
	})
})
