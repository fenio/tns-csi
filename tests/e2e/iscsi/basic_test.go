...
var _ = Describe("iSCSI Mount and Expansion", func() {
	...
	It("should provision an iSCSI volume, mount it as filesystem, perform I/O, and expand it", func() {
		...
		_, err = f.K8s.ExecInPod(ctx, pod.Name, []string{"sh", "-c", "echo 'hello iscsi' > /data/test.txt"})
		...
		Expect(output).To(ContainSubstring("hello iscsi"))

		By("Verifying filesystem is ext4 (default for iSCSI)")
