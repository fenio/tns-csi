	...
	It("should create truly independent detached snapshot (no ZFS clone dependency)", func() {
		...
		if err != nil || !exists {
			if contentInfo.SnapshotHandle != "" {
				GinkgoWriter.Printf("[NFS] Detached dataset not at expected path, checking handle...\n")
			}
		}
		...
		if err != nil {
			GinkgoWriter.Printf("[NFS] WARNING: Could not check clone status: %v\n", err)
			...
			if queryErr := f.TrueNAS.Client().Call(ctx, "pool.dataset.query", []any{
				...
				for _, ds := range allDatasets {
					if name, ok := ds["id"].(string); ok && strings.Contains(name, snapshotName) {
						...
						GinkgoWriter.Printf("[NFS] Found detached dataset: %s\n", detachedDatasetPath)
						break
					}
				}
			}
