#!/bin/bash
# NVMe-oF Volume Clone Integration Test
...
PROTOCOL="NVMe-oF Volume Clone"
SOURCE_PVC_NAME="test-pvc-source-nvmeof"
SOURCE_POD_NAME="test-pod-source-nvmeof"
CLONE_PVC_NAME="test-pvc-clone-nvmeof"
CLONE_POD_NAME="test-pod-clone-nvmeof"
MANIFEST_DIR="${SCRIPT_DIR}/manifests"

# Check if test should be skipped based on tags
if should_skip_test "clone,nvmeof"; then
    echo "Skipping NVMe-oF Volume Clone test (tag-based skip)"
    exit 0
fi

echo "========================================"
echo "TrueNAS CSI - NVMe-oF Volume Clone Test"
echo "========================================"
...
trap 'save_diagnostic_logs "nvmeof-clone" "${CLONE_POD_NAME}" "${CLONE_PVC_NAME}" "/tmp/test-logs"; show_diagnostic_logs "${CLONE_POD_NAME}" "${CLONE_PVC_NAME}"; cleanup_clone_test; test_summary "${PROTOCOL}" "FAILED"; exit 1' ERR
...
test_clone_restore() {
    ...
    kubectl apply -f "${MANIFEST_DIR}/pvc-clone-from-volume-nvmeof.yaml" -n "${TEST_NAMESPACE}"
...
if ! check_nvmeof_configured "${MANIFEST_DIR}/pvc-source-nvmeof.yaml" "${SOURCE_PVC_NAME}" "${PROTOCOL}"; then
    exit 0  # Exit gracefully - test was skipped
fi

# Create source PVC
create_pvc "${MANIFEST_DIR}/pvc-source-nvmeof.yaml" "${SOURCE_PVC_NAME}"

# Create source pod and write test data
create_test_pod "${MANIFEST_DIR}/pod-source-nvmeof.yaml" "${SOURCE_POD_NAME}"
