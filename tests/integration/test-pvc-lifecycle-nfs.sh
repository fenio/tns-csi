...
delete_pvc_directly() {
    ...
    echo ""
    test_info "Verifying dataset exists on TrueNAS BEFORE deletion..."
    ...
    test_success "Confirmed dataset exists on TrueNAS before deletion"
    ...
    # CRITICAL: Verify the dataset was actually deleted from TrueNAS
    # This is the key test - PV deletion alone doesn't prove backend cleanup
    echo ""
    test_info "Verifying dataset was deleted from TrueNAS backend..."
    if ! verify_truenas_deletion "${dataset_name}" 30; then
        test_error "TrueNAS dataset still exists! CSI DeleteVolume did not clean up backend."
        echo ""
        echo "=== Listing all CSI datasets on TrueNAS ==="
        list_truenas_datasets || true
        echo ""
        ...
        kubectl logs -n kube-system \
            -l app.kubernetes.io/name=tns-csi-driver,app.kubernetes.io/component=controller \
            --tail=100 | grep -i -E "(delete|volume|dataset)" || true
        stop_test_timer "delete_pvc_directly" "FAILED"
        false
    fi
    test_success "TrueNAS dataset confirmed deleted - backend cleanup verified!"
