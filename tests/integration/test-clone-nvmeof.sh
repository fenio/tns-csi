...
# Tests PVC-to-PVC cloning for NVMe-oF volumes
#
...
PROTOCOL="NVMe-oF Clone"
SOURCE_PVC_NAME="test-pvc-nvmeof"
SOURCE_POD_NAME="test-pod-nvmeof"
...
echo "========================================"
echo "TrueNAS CSI - NVMe-oF Clone Test"
echo "========================================"
...
trap 'save_diagnostic_logs "nvmeof-clone" "${SOURCE_POD_NAME}" "${SOURCE_PVC_NAME}" "/tmp/test-logs"; show_diagnostic_logs "${SOURCE_POD_NAME}" "${SOURCE_PVC_NAME}"; cleanup_test "${SOURCE_POD_NAME}" "${SOURCE_PVC_NAME}"; test_summary "${PROTOCOL}" "FAILED"; exit 1' ERR
...
create_and_verify_clone() {
    ...
    # Show manifest
    show_yaml_manifest "${MANIFEST_DIR}/pvc-clone-nvmeof.yaml" "Clone PVC Manifest"
    
    # Apply clone PVC
    kubectl apply -f "${MANIFEST_DIR}/pvc-clone-nvmeof.yaml" -n "${TEST_NAMESPACE}"
...
if ! check_nvmeof_configured "${MANIFEST_DIR}/pvc-nvmeof.yaml" "${SOURCE_PVC_NAME}" "${PROTOCOL}"; then
    exit 0  # Graceful skip - NVMe-oF not configured
fi

create_pvc "${MANIFEST_DIR}/pvc-nvmeof.yaml" "${SOURCE_PVC_NAME}"
create_test_pod "${MANIFEST_DIR}/pod-nvmeof.yaml" "${SOURCE_POD_NAME}"
