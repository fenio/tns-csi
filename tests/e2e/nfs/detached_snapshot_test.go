...
var _ = Describe("NFS Detached Snapshot", func() {
	var f *framework.Framework
	var ctx context.Context
	var err error

	// Timeouts for NFS operations
	const (
	...
	It("should create detached clone that survives snapshot deletion", func() {
		...
		testData := "Detached Snapshot Test Data - NFS"
		...
		snapshotClassName := "test-snapshot-class-nfs"
		...
		_, err = f.K8s.ExecInPod(ctx, detachedPodName, []string{
			"sh", "-c", fmt.Sprintf("echo '%s' > /data/detached.txt", detachedData),
		})
		Expect(err).NotTo(HaveOccurred())
		...
		_, err = f.K8s.ExecInPod(ctx, detachedPodName, []string{
			"sh", "-c", fmt.Sprintf("echo '%s' > /data/post-delete.txt", postDeleteData),
		})
		Expect(err).NotTo(HaveOccurred())
