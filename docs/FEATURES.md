...
- **Platform**: GitHub Actions — `integration.yml` runs on GitHub-hosted `ubuntu-24.04` runners (k3s in QEMU VM); auxiliary workflows (encryption, scale, stress, distro-compatibility) currently disabled, pending QEMU migration
- **Workflows**:
...
- **Infrastructure**: k3s in a QEMU VM on a GitHub-hosted runner (for `integration.yml`) + real TrueNAS over Tailscale; auxiliary workflows currently disabled, pending QEMU migration
- **Test Scenarios**:
...
- ✅ **Tested**: k3s (in QEMU VM on GitHub-hosted CI for `integration.yml`; auxiliary workflows currently disabled, pending QEMU migration)
...
**Test Environment:**
- ✅ GitHub Actions CI (GitHub-hosted `ubuntu-24.04` + QEMU/k3s for `integration.yml`; auxiliary workflows currently disabled, pending QEMU migration)
