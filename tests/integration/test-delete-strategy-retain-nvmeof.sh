#!/bin/bash
# Delete Strategy Retain Integration Test for NVMe-oF
# Tests that deleteStrategy=retain keeps TrueNAS resources after PV deletion
#
# This test ports the NFS deleteStrategy=retain test to NVMe-oF protocol
...
PROTOCOL="NVMe-oF Delete Strategy Retain"
PVC_NAME="test-pvc-nvmeof-retain"
POD_NAME="test-pod-nvmeof-retain"
STORAGECLASS_NAME="tns-csi-nvmeof-retain"

# Check if test should be skipped based on tags
if should_skip_test "retain,nvmeof"; then
    echo "Skipping Delete Strategy Retain NVMe-oF test (tag-based skip)"
    exit 0
fi

echo "========================================"
echo "TrueNAS CSI - Delete Strategy Retain Test (NVMe-oF)"
echo "========================================"
...
trap 'save_diagnostic_logs "nvmeof-retain" "${POD_NAME}" "${PVC_NAME}" "/tmp/test-logs"; show_diagnostic_logs "${POD_NAME}" "${PVC_NAME}"; cleanup_retain_test; test_summary "${PROTOCOL}" "FAILED"; exit 1' ERR

#######################################
# Create StorageClass with deleteStrategy=retain for NVMe-oF
#######################################
create_retain_storageclass() {
    test_step "Creating StorageClass with deleteStrategy=retain (NVMe-oF)"
...
test_retain_behavior() {
    ...
    # Parse volume handle to get TrueNAS ZVOL path
    # NVMe-oF format is typically: protocol#server#zvolPath
    local zvol_path
    zvol_path=$(echo "${volume_handle}" | cut -d'#' -f3)
    test_info "TrueNAS ZVOL path: ${zvol_path}"
    ...
    if is_verbose 1; then
        ...
        echo "${logs}" | grep -E "DeleteVolume|retain|zvol|subsystem|namespace" || echo "No matching log entries found"
    fi
    ...
    test_info "ZVOL path that should still exist: ${zvol_path}"
}
...
# Check if NVMe-oF is configured on TrueNAS using a temporary manifest
echo ""
test_info "Checking if NVMe-oF is configured on TrueNAS..."

# Create a temporary PVC to check NVMe-oF configuration
cat <<EOF > /tmp/nvmeof-retain-check-pvc.yaml
apiVersion: v1
kind: PersistentVolumeClaim
metadata:
  name: nvmeof-retain-check
spec:
  accessModes:
    - ReadWriteOnce
...
if ! check_nvmeof_configured "/tmp/nvmeof-retain-check-pvc.yaml" "nvmeof-retain-check" "${PROTOCOL}"; then
...
# Wait for PVC to be bound (now that pod is created)
...
kubectl exec "${POD_NAME}" -n "${TEST_NAMESPACE}" -- \
    sh -c "echo 'Retain Test Data NVMe-oF' > /data/retain-test.txt"
