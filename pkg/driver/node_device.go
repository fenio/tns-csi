...
func needsFormatWithRetries(ctx context.Context, devicePath string, isClone bool) (bool, error) {
	...
	return handleFinalResult(devicePath, maxRetries, lastOutput, lastErr, isClone)
}
...
func formatDevice(ctx context.Context, volumeID, devicePath, fsType string) error {
	...
	case fsTypeXFS:
		...
		// backing zvol's physical sector size differs from the transport's
		// advertised logical sector size (common with NVMe-oF over TCP).
		args := []string{"-f"}
		if sectorSize, err := getLogicalSectorSize(devicePath); err == nil {
			args = append(args, "-s", fmt.Sprintf("size=%d", sectorSize))
